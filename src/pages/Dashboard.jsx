...
import QuickStatsWidget, { createEnergyStats } from '../components/QuickStatsWidget';
import DashboardWidget, { WidgetContainer } from '../components/DashboardWidget';
import { calculateBackupTime, calculateEfficiencyRating } from '../utils/format';
...
const Dashboard = () => {
  ...
  const backupTime = calculateBackupTime(batteryLevel, currentConsumption, batteryCapacity) || "5.2 hours";
  const efficiencyData = calculateEfficiencyRating(currentConsumption, expectedConsumption, 85);
    ...
    <div className="min-h-screen bg-gray-50 p-3 sm:p-4">
      ...
      >
        ...
        <StatusCard
          ...
          icon="⚡"
          color={efficiencyData.color === 'green' ? "status-online" : efficiencyData.color === 'yellow' ? "status-warning" : "status-error"}
        />
      </motion.div>
      ...
      >
        {/* Section Header */}
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Enhanced Dashboard Widgets</h2>
          <p className="text-gray-600 mt-1">Advanced monitoring and control components</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <AdvancedStatusCard
            ...
            progress={{ current: state.batteryLevel || 75, max: 100, showBar: true }}
            chart={{ type: 'donut', data: [state.batteryLevel || 75, 100] }}
            actions={[
              { label: 'Battery Details', onClick: () => console.log('Battery details') },
              { label: 'Health Check', onClick: () => console.log('Health check') }
            ]}
            showRefresh={true}
          ...
          <AdvancedStatusCard
            ...
            actions={[
              { label: 'Usage History', onClick: () => console.log('Usage history') },
        ...
        <WidgetContainer columns="repeat(auto-fit, minmax(350px, 1fr))" className="mb-8">
          <DashboardWidget
          ...
          >
            <div className="space-y-4">
              <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                <h4 className="font-semibold text-gray-900 mb-2">⏱️ Backup Time Analysis</h4>
                <div className="text-2xl font-bold text-blue-600 mb-1">{backupTime}</div>
                <p className="text-sm text-gray-600">
                  Based on current consumption of {currentConsumption}W and battery level of {batteryLevel}%
                </p>
                <div className="mt-2 text-xs text-gray-500">
                  Battery Capacity: {batteryCapacity}Ah | Remaining: {Math.round((batteryLevel / 100) * batteryCapacity)}Ah
                </div>
              </div>
              
              <div className="p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                <h4 className="font-semibold text-gray-900 mb-2">⚡ Efficiency Rating</h4>
                <div className="flex items-center space-x-2 mb-1">
                  <div className="text-2xl font-bold text-green-600">{efficiencyData.percentage}%</div>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    efficiencyData.color === 'green' ? 'bg-green-100 text-green-800' :
                    efficiencyData.color === 'yellow' ? 'bg-yellow-100 text-yellow-800' :
                ...
                <p className="text-sm text-gray-600">
                  Current: {currentConsumption}W | Expected: {expectedConsumption}W
                </p>
                <div className="mt-2 text-xs text-gray-500">
